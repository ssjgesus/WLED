...
void WLED::setup()
{
  ...
  escapedMac = "000000000000"; // use the eFuse Ethernet MAC below without starting Wi-Fi
#else
  ...
  WiFi.onEvent(WiFiEvent); // register before config loading can start Ethernet
#endif
  // AI: end
...
{
#ifdef WLED_USE_ETHERNET_ONLY
  return; // this build does not start ESP-Hosted Wi-Fi
#else
  #if defined(WLED_USE_ETHERNET)
  if (ethernetOnly) return; // runtime Ethernet-only mode does not start Wi-Fi AP
  #endif
#endif
...
#else
  #if defined(WLED_USE_ETHERNET)
  if (ethernetOnly) return; // runtime Ethernet-only mode does not start Wi-Fi
  #endif
#endif
...
{
#if defined(WLED_USE_ETHERNET)
  // Ethernet-only mode uses the Ethernet driver's link/IP state and never
  // enters the Wi-Fi reconnect/AP state machine.
  if (ethernetOnly) {
  // The driver handles link recovery and DHCP; restart WLED services after IP acquisition.
