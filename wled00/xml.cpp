...
static void appendGPIOinfo(Print& settingsScript)
{
  ...
  if (ethernetType != WLED_ETH_NONE && ethernetType < WLED_NUM_ETH_TYPES) {
    ...
    settingsScript.print(ETH_RMII_CLK);
#else
...
void getSettingsJS(byte subPage, Print& settingsScript)
{
  ...
  if (subPage == SUBPAGE_WIFI)
  {
    ...
    // AI: below section was generated by an AI
    #if CONFIG_IDF_TARGET_ESP32P4
    settingsScript.print(F("for(const o of [...d.Sf.ETH.options]){if(o.value!=='0'&&o.value!=='7')o.remove();else if(o.value==='7')o.text='ESP32-P4 IP101';}"));
    #endif
    // AI: end
    printSetFormValue(settingsScript,PSTR("ETH"),ethernetType);
    printSetFormCheckbox(settingsScript,PSTR("ETHO"),ethernetOnly);
    #if defined(WLED_USE_ETHERNET_ONLY)
    // This profile has Wi-Fi removed from its startup path, so the setting is
    // shown as enabled but cannot be turned off accidentally.
    settingsScript.print(F("gId('ETHO').disabled=true;"));
    #endif
    #else
    //hide ethernet setting if not compiled in
    settingsScript.print(F("gId('ethd').style.display='none';gId('etho').style.display='none';"));
    #endif
