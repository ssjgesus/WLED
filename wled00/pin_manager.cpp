...
bool PinManager::deallocatePin(byte gpio, PinOwner tag)
{
  ...
  if (gpio >= WLED_NUM_PINS || (!isPinOk(gpio, false) && tag != PinOwner::Ethernet)) return false; // but return false for any other invalid pin
...
bool PinManager::allocateMultiplePins(const managed_pin_type * mptArray, byte arrayElementCount, PinOwner tag)
{
  ...
  for (int i = 0; i < arrayElementCount; i++) {
    ...
    if (gpio >= WLED_NUM_PINS) return false;
    // AI: end
    ...
    if ((tag==PinOwner::HW_I2C || tag==PinOwner::HW_SPI) && isPinAllocated(gpio, tag)) {
      ...
      continue;
    } else if (tag == PinOwner::Ethernet ? bitRead(pinAlloc, gpio) : isPinAllocated(gpio)) {
